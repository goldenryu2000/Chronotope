...
export default async function Page(props: PageProps<'/[region]/[pack]'>) {
  ...
  // The tree of atlases is resolved here for the same reason the pack list is:
  // the browser never asks the database anything. It is one query over a
  // handful of rows this request has already warmed, and it is not in the
  // region artifact, because the answer changes when a *pack* is published and
  // the artifact is immutable.
  const [regionUrl, packUrl, siblings, regionLayers, plates] = await Promise.all([
